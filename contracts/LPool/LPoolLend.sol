...
abstract contract LPoolLend is LPoolApproved {
    ...
    function loan(IERC20 token_, uint256 amount_)
        external
        ...
        onlyPA(token_)
    {
        require(
            ...
            "Withdraw amount exceeds available liquidity"
        );
        token_.safeTransfer(_msgSender(), amount_);
        _loaned[_msgSender()][token_] = _loaned[_msgSender()][token_].add(
            amount_
        );
    ...
    function repay(IERC20 token_, uint256 amount_)
        external
        onlyRole(POOL_APPROVED)
        onlyPA(token_)
    {
        token_.safeTransferFrom(_msgSender(), address(this), amount_);
        if (amount_ > _loaned[_msgSender()][token_])
            _loaned[_msgSender()][token_] = 0;
        else
            _loaned[_msgSender()][token_] = _loaned[_msgSender()][token_].sub(
                amount_
            );
    ...
    function loaned(IERC20 token_, address account_)
        external
        view
        returns (uint256)
    {
