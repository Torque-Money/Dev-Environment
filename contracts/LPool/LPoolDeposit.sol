...
abstract contract LPoolDeposit is LPoolApproved, LPoolTax {
    ...
    function deposit(IERC20 token_, uint256 amount_)
        external
        onlyRole(POOL_APPROVED)
        onlyPA(token_)
    {
        (
            uint256 taxPercentNumerator,
            uint256 taxPercentDenominator
        ) = taxPercentage();
        uint256 tax = taxPercentNumerator.mul(amount_).div(
            taxPercentDenominator
        );
    ...
    function withdraw(IERC20 token_, uint256 amount_)
        external
        onlyRole(POOL_APPROVED)
        onlyPA(token_)
    {
