...
async function main() {
    ...
    const lens = await hre.ethers.getContractAt("Lens", data.Lens.proxies[0]);
    ...
    const strategies = data.BeefyLPStrategy.proxies;
    ...
    await (await lens.grantRole(LENS_CONTROLLER_ROLE, newAdmin)).wait();
    ...
    await (await lens.grantRole(REGISTRY_ADMIN_ROLE, newAdmin)).wait();
    ...
    await (await lens.grantRole(LENS_ADMIN_ROLE, newAdmin)).wait();
