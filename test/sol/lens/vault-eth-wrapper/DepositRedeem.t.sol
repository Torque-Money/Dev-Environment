...
contract DepositRedeemTest is BaseVault {
    ...
    function testDepositRedeem() public useFunds(vm) {
        ...
        for (uint256 i = 0; i < _token.length; i++)
            if (address(_token) == address(Config.getWETH())) {
                ethDeposit = amount[i];
                break;
            }

        uint256 shares = _wrapper.deposit{value: ethDeposit}(_tokenAmount);

        // Check balances have been updated
        for (uint256 i = 0; i < _token.length; i++)
            if (address(_token) != address(Config.getWETH())) _assertEq(_token.balanceOf(address(this)), initialBalance[i].sub(_tokenAmount[i]));
        ...
        for (uint256 i = 0; i < _token.length; i++)
            if (address(_token) != address(Config.getWETH())) _assertApproxEq(_token.balanceOf(address(this)), initialBalance[i]);
            else assertApproxEq(address(this).balance, initialETH);
