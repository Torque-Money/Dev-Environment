...
contract DepositRedeemTest is BaseVault {
    ...
    function testDepositRedeem() public useFunds(vm) {
        ...
        uint256 expectedShares = _vault.estimateDeposit(tokenAmount);
        ...
        for (uint256 i = 0; i < token.length; i++) _assertApproxEq(_vault.approxBalance(token[i]), tokenAmount[i]);
    ...
    function testDepositRedeemZero() public useFunds(vm) {
        ...
        uint256[] memory out = _vault.redeem(_vault.deposit(tokenAmount));
    ...
    function testDepositRedeemWithTokenInjection() public useFunds(vm) {
        ...
        for (uint256 i = 0; i < token.length; i++) token[i].safeTransfer(address(_vault), tokenAmount[i]);
    ...
    function testDepositRedeemMultiple() public useFunds(vm) {
        ...
        uint256 shares0 = _vault.deposit(tokenAmount);
        ...
        vm.startPrank(_empty);
        {
            ...
            uint256 shares1 = _vault.deposit(tokenAmount);
